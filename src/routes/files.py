...
@router.post("/")
async def add_file(files: list[UploadFile], db: Session = Depends(get_db),
...
async def delete_file(document_id: int = Path(ge=1), db: Session = Depends(get_db),
                      current_user: User = Depends(auth_service.get_current_user)):
    deleted_count = await repository_files.delete_document_by_id(document_id, current_user.id, db)
